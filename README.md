...
Z koreňa repozitára alebo z priečinka príkladu:
...
Otvor v prehliadači adresu z výstupu (predvolene `http://127.0.0.1:3000` pre prvý príklad). Druhý a tretí príklad používajú iné porty (`3010`, `3020`), aby sa dali spúšťať paralelne.
...
| Priečinok | Balík (crate) | Port |
|-----------|---------------|------|
| [`examples/01-hello-ssr`](examples/01-hello-ssr) | `hello_ssr` | 3000 |
| [`examples/02-counter`](examples/02-counter) | `counter` | 3010 |
| [`examples/03-server-fn-example`](examples/03-server-fn-example) | `server_fn_example` | 3020 |
...
- **Symfony routing / controller** — v týchto demách **Axum** `Router`: URL cesta ide na **handler** (`async fn`), podobná myšlienka ako „route → akcia“, len bez `front controller` v štýle starého `app.php`.
...
- **MySQL** — tieto tri príklady **nemajú databázu** (držíme kód krátky). Typický ďalší krok v Ruste je napr. **SQLx** alebo **Diesel** s **Tokio** poolom — analogicky k PDO/Doctrine alebo SQLAlchemy.
